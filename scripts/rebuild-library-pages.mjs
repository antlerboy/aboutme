...
function card(item) {
  return `<article class="card" data-tags="${h((item.tags || []).join(' '))}">${item.kicker ? `<p class="eyebrow">${h(item.kicker)}</p>` : ''}<h3>${h(item.title)}</h3><p>${h(item.description)}</p>${tagList(item.tags)}<a class="go" href="${h(item.url)}">${h(item.action || 'Open')} →</a>${item.source ? `<a class="go" href="${h(item.source)}">${h(item.sourceLabel || 'Presentation details')} →</a>` : ''}</article>`;
}
...
const waysIn = [
  ...
  { title: 'Talks, teaching, and workshops', description: 'Public talks, workshops, and presentation slides.', tags: ['practice'], url: '/library/talks-and-sessions/' },
...
save('', shell({
  ...
  lead: 'Public writing, talks, tools, models, recordings, and reference material from my work in systems practice and public services.',
...
const scio = [
  { kicker: 'April 2014 · SCiO', title: 'Power+Systems: systems blindness and systems sight', description: 'An introduction to Barry Oshry’s approach to patterns of power and experience in organisations.', tags: ['systems', 'leadership', 'scio'], url: 'https://www.systemspractice.org/resources/powersystems-systems-blindness-and-systems-sight', action: 'View presentation details' },
...
save('talks-and-sessions', shell({ slug: 'talks-and-sessions', title: 'Talks, teaching, and workshops', description: 'Public sessions and presentation slides.', eyebrow: 'Public sessions', lead: 'Public talks, workshops, and presentation slides.', body: cardSection('Recent public sessions', '', recentTalks, { toolbar: true }) + cardSection('StretchCon presentations', 'Two talks on leadership, organisation, and the conditions for effective teams.', readerRecordings.recordings.filter(item => /Stretch/.test(item.kicker))) + cardSection('Public SCiO sessions', 'Recordings, workshop resources, and presentation slides.', scio, { alt: true }) + cardSection('Presentation slides', 'Download slides on systems practice, leadership, and public-service change.', suppliedTalks), scripts: ['/library/app.js'] }));
...
const podcasts = [
  ...
  { title: 'The Human Current 040: The myth of the dysfunctional system', description: 'Benjamin Taylor contributes to a discussion of organisational dysfunction and the results systems produce.', tags: ['audio', 'systems'], url: 'https://soundcloud.com/humancurrent/040-the-myth-of-the' },
  { title: 'The Human Current 038: A philosophical look at company culture', description: 'A philosophical look at company culture.', tags: ['audio'], url: 'https://soundcloud.com/humancurrent/038-a-philosophical-look-at' },
